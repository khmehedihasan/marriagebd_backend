...
exports.addPayment = async (req, res, next) => {
  try {
    ...
    if (data.id != undefined) {
      const dd = await User.findByIdAndUpdate(req.params.id, {
        $push: { payments: d._id },
      });
...
exports.getAllPayment = async (req, res, next) => {
  try {
    ...
    result.data = await Payment.find()
      .select({ password: 0, token: 0, verificationCode: 0 })
      .populate("user", "name phone uid packageValidity")
      .limit(limit)
      .skip(skip)
      .sort({ date: -1 });
...
exports.getAllPaymentById = async (req, res, next) => {
  try {
    ...
    const skip = (page - 1) * limit;
    const result = {};
    result.totalData = await Payment.find({
      user: req.params.id,
    }).countDocuments();
    result.data = [];
    ...
    result.data = await Payment.find({ user: req.params.id })
      .select({ password: 0, token: 0, verificationCode: 0 })
      .populate("user", "name phone uid packageValidity")
      .limit(limit)
      .skip(skip)
...
exports.searchPayment = async (req, res, next) => {
  try {
    const dcount = await Payment.find({
      $or: [
  ...
  try {
    ...
    if (d == null) {
      ...
      if (data == null) {
        await res
          .status(400)
          .send({ status: false, message: "Faild to delete payment." });
      } else {
        await User.findByIdAndUpdate(data.user._id, {
          $pull: { payments: data._id },
        });
